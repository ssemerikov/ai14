...
fun makevector(size, value)=
  if size>0 then
      ...
      raise SizeError "Nedozvolennaya razmernost vektora"

fun makematrix(m,n,value)=
  if m>0 andalso n>0 then
      Array2.array(m, n, value)
  else
      raise SizeError "Nedozvolennaya razmernost matritsyi"
...
fun main()=
let 
  val NUMPAT=ref 1024  (*число обучающих шаблонов - может переопределяться в файле*)
...
in 
  ...
  makenetwork(!NUMIN,!NUMOUT,!NUMHID);
  print("Razmernost vhoda - " ^ Int.toString(!NUMIN) ^ ", razmernost vyihoda - " ^  Int.toString(!NUMOUT) ^ 
        ", chislo shablonov - " ^ Int.toString(!NUMPAT)^ "\n");
  train(Input,Output,0.000001,150000, true, "network.txt")
  (*readnetwork("network.txt")*)
  
  (*print "Ishodnyie dannyie:\n";
  i := 0;
  ...
  (
    ...
    print " Etalonnyiy vyihod: \n";
    printvector (vectorfrommatrix(Output, !i));
    print " Poluchennyiy vyihod: (Nauka, Sport, Ekonomika, Politika, Kul'tura)\n";
    printvector(!res);
