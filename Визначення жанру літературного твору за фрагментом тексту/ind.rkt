...
(define (train TrainInput TrainTarget Err MaxCount DoOut NetworkFile)
  ...
  (let* (
        ...
        (do [(epoch 0 (+ 1 epoch)) ] ( (or (= epoch MaxCount) (< Error Err)) (write-network NetworkFile) Error)            
          ...
          (when (and DoOut (= (remainder epoch 500) 0));отладочный вывод
      ...
      (let ((str s))
        (do ((i 0 (+ i 1)))
          ...
          (if (>= i (string-length s))
              (array-set! v i 32)
              (array-set! v i (char->integer (string-ref s i)))
              )
          )
        )
      (error "not string")
...
(define (main)
  ...
  ;(train Input Output 0.01 2000 #t "network.txt")
  ...
  (do [(i 0 (+ 1 i)) (res null)] ((= i count))
        ...
        (printf " Полученный выход (Детектив, Триллер, Юмористическая проза, Роман, Фантастика):")
