...
(define I (make-matrix 1 2))
(define Вікно (new frame%       [label "Прогнозування результатів матчів АПЛ"])) 
...
(define Панель (new horizontal-panel%       [parent Вікно]       [alignment '(center center)]))
(define Загрузка (new button% 
[parent Панель] 
[label "Завантаження вагових коефіцієнтів"] 
[min-width 200]
[min-height 20]
[callback (lambda (button event) 
(begin 
(read-network "network.txt") 
(send Повідомлення set-label "Коефіцієнти завантаженні") 
) 
)]))
(define Обучение (new button% 
[parent Панель] 
...
(begin 
(main) 
(send Повідомлення set-label "Навчання пройшло успішно") 
) 
)]))


(define operation1 (new choice% [parent Вікно] 
[label "Оберіть 1 команду: "] 
[choices '("Манчестер Сити"
"Манчестер Юнайтед"
"Челси"
"Ливерпуль"
"Тоттенхэм Хотспур" 
...
"Суонси Сити" )]))
(define operation2 (new choice% [parent Вікно] 
[label "Оберіть 2 команду: "] 
[choices '("Манчестер Сити"
"Манчестер Юнайтед"
"Челси"
"Ливерпуль"
...
[min-width 400]
[callback (lambda (button event)
            (define team1 (send operation1 get-selection))
            (define team2 (send operation2 get-selection))
            (define res 
(begin
  (setmvalue I 0 0 (abs (+ 1 team1)))
  
  (setmvalue I 0 1 (abs (+ 1 team2)))
  ;(set! I (make-array (shape 0 (abs (+ 1 team1)) 0 (abs ( + 1 team2)))0))  

   ))
            (and (read-network "network.txt") (start)))
            ]))
...
(define (start)
  (if (= 0 NUMHID)
      ...
      (let ( (in (make-vector  NUMIN)) (res 0)  )
        (set! res (getoutput (vector-from-matrix I 0)))
   
        (newline)
        (message-box "Полученный выход :" (format  "За прогнозом матч закінчиться з рахунком  ~n ~a : ~a"  (array-ref res 0) (array-ref res 1 )) #f '(ok)) ;(printf (vector-from-matrix res -1)))
        (print-vector res)
        (newline)
