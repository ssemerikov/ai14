...
"слезоточение"
"элементы сыпи"
"низкое давление (на 30-50 мм рт.ст. от нормы пациента)"
"ВЫСОКОЕ давление (на 10-50 мм рт.ст. от нормы пациента)"
...
"боли в животе после приема жирной пищи и алкоголя"
...
(define (болезнь res)
  (let ([max (array-ref res 0)] [index 0])
    (do ((i 0 (1+ i)) )
      ((= i (array-end res 0)) (format "Предполагаемый диагноз: ~A (с вероятностью ~A)\n" (list-ref список-болезней index) max ))
...
(define (menu)
  (display "Справочник семейного врача (терапевта)")
  (newline)
  (display "Материалы взяты из 'Рациональная диагностика и фармакотерапия заболеваний внутренних органов' О.Я. Бабак")
  (newline)
  (display "Для начала работы выберите один из пунктов:")
  (newline)
  (display "1.Обучение сети")
  (newline)
  (display "2.Определение диагноза по основным ссимптомам")
  (newline)
  (display "3.Завершение работы со справочником")
  (newline)
  (define choice (read))
  (cond
    ((= choice 1) (main))
