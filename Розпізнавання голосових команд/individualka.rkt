...
(define Вікно (new frame% 
                   [label "Розпізнавання голосових команд"]
                   [width 540] 
                   [height 300] 
                   [x 100] 
                   [y 150] 
...
;                            [callback (lambda (button event) (record-sound))]
;                            
...
(define Кнопка-розпізнати (new button% 
                              ...
                              ;[callback (lambda (button event) (send Рамка show #t))]
                               ;[callback (lambda (button event) (soundrec))]
                                 [callback (lambda (button event)
                                 
                                 (send Повідомлення set-label "старт! Говоріть...")
                                             (soundrec)
                                 (send Повідомлення set-label "записано")   
                                 )
                               ]
                                 
             
              [label "2. Розпізнати команду"]))
 (define Повідомлення (new message%
                          [parent Панель-справа]
...
(define s (record-sound (* (default-sample-rate) 2)))
(printf "... done\n")
...
(define size (file-size "D:/1.wav"))
 (define what (open-input-file "d:/1.wav" #:mode 'binary))
  (define f (open-output-file "d:/1.txt" #:mode 'binary   #:exists 'replace) )
...
(define (train TrainInput TrainTarget Err MaxCount DoOut NetworkFile)
  ...
  (let* (
        ...
        (do [(epoch 0 (+ 1 epoch)) ] ( (or (= epoch MaxCount) (< Error Err)) (write-network NetworkFile) Error)            
          ...
          (when (and DoOut (= (remainder epoch 1024) 0));отладочный вывод
