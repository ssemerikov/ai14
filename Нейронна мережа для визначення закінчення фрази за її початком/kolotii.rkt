...
(define (readdata v)
  (display "Слово:\n")
  (display "Введите 1, если вы хотите использовать это слово, и 0, если нет: ")
  (newline)
  (do ((i 0 (1+ i)) )
    ((= i (array-end v 0))   v)
...
"от белки бежать"
"лес не ходить"
"а сердце ýже заячьей лапы"
"да издалека"
"теляти да волка поймать"
"на осине"
"ушла"
"тот и пенька в лесу боится"
"тот честью не дорожит"
;"коротко а сабля не вынимается"
;"а на молодцá  сам овца"
;"уж  змея"
;"собак"
;"тени боится"
;"пару не бояться"
...
;"за неправдой ходит"
;"жизнь одна"
;"коня сзади а лихого человека  со всех сторон"


                          ))
(define (print-список-болезней v)
  (do ([i 0 (+ i 1)])
    ((= i (array-end v 0)))
    (printf
...
(define (болезнь res)
  (let ([max (array-ref res 0)] [index 0])
    (do ((i 0 (1+ i)) )
      ((= i (array-end res 0)) (format "Возможное окончание фразы: ~A (с вероятностью ~A)\n" (list-ref список-болезней index) max ))
...
(define (menu)
  (display "Нейронная сеть для определения окончания фразы по ее началоу")
  (newline)
  (display " выберите один из вариантов, поставив соответствующую цифру:")
  (newline)
  (display "1 Обучить сеть")
  (newline)
  (display "2 определения окончания фразы по ее началоу")
  (newline)
  (display "3 Завершение работы программы")
  (newline)
  (define choice (read))
  (cond
    ((= choice 1) (main))
