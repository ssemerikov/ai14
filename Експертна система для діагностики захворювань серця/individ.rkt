...
(define NUMIN 24)  ;размерность входа
(define NUMHID 22) ;размерность скрытого слоя
(define NUMOUT 22) ;размерность выхода
(define NUMPAT 60) ;количество шаблонов для обучения 
...
;(set! NUMIN  18)  ;размерность входа - может переопределяться в файле
;(set! NUMHID 14) ;число нейронов в скрытом слое
;(set! NUMOUT 14)  ;размерность выхода - может переопределяться в файле
...
(define (main)
  ...
  (train Input Output 0.00001 150000 #t)
...
(define (menu)
  ...
  (newline)
  (display "2. Определить диагноз по симптомам")
  (newline)
  (display "3. Выход")
  (newline)
  (define choice (read))
